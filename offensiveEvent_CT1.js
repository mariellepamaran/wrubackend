        ...
        if(method === "DELETE"){
            ...
            dispatchCollection.aggregate([
                ...
                var _ids = {
                    ...
                    queueingAtOrigin: [],
                    processingAtOrigin: [],
                    queueingAtDestination: [],
                    processingAtDestination: [],
                    complete: []
                },
                dispatch = {};
                ...
                if(docs.length > 0){
                    saveVehicleLocation(function(){
                        for(var i = 0; i < docs.length; i++){
                            ...
                            // origin
                            if(getIndexOf(["Inside Geofence","Queueing"],"and") && doc.status == "dispatch" && isOrigin === true){ //  && query.stage == "end"
                                _ids.queueingAtOrigin.push(doc._id);
                            }
                            if(getIndexOf(["Inside Geofence","Processing"],"and") && ["dispatch","queueingAtOrigin"].includes(doc.status) && isOrigin === true){ //  && query.stage == "end"
                                _ids.processingAtOrigin.push(doc._id);
                            }
                            // end origin
    
                            // in transit
                            if(query.RULE_NAME == "Inside Geofence" && ["dispatch","queueingAtOrigin","processingAtOrigin"].includes(doc.status) && isOrigin === true && query.stage == "end"){
