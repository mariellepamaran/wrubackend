    ...
    co(function*() {
        ...
        if(req.headers.authorization === "395d8ef62b8a4de" && userId){
            ...
            if (method == 'OPTIONS') {
                ...
                if(parameterComplete(["name","email","phoneNumber"],body)) {
                    var docs = yield usersCollection.insertOne({
                        _id: body._id,
                        name: body.name,
                        email: body.email,
                ...
                if(parameterComplete(["name","email","phoneNumber"],body) && parameterComplete(null,params_value)) {
                    var _id = params_value[1];
                    if(isEmpty([userId,_id]) === true){
                        res.status(400).send('Error: Missing parameters');
                    } else {
                        var docs = yield usersCollection.updateOne(
                            { _id }, 
                            {
                                $set: {
                                    name: body.name,
                ...
                if(parameterComplete(null,params_value)) {
                    ...
                    if(isEmpty([userId,_id]) === true){
                        ...
                        var docs = yield usersCollection.deleteOne({_id});
                        closeConnection(docs);
                    }
                } else {
                    res.status(400).send('Error: Missing parameters');
