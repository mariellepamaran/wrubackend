                ...
                if(docs.length > 0){
                    saveVehicleLocation(function(){
                        ...
                        for(var i = 0; i < docs.length; i++){
                            ...
                            if(!doc.scheduled_date || (moment().valueOf(new Date()) >= moment(doc.scheduled_date).valueOf())){
                                        // if already has entered_origin
                                if((!doc.event_time || (doc.event_time && !doc.event_time.entered_origin)) && 
                        ...
                        function proceedToUpdate(){
                            ...
                            Object.keys(_ids).forEach(function(status) {
                                if(_ids[status].length > 0){
                                    var set = {};
                                    if(!["entered_origin","idle_origin"].includes(status)){
                                        set["status"] = status;
