        ...
        var client = yield mongodb.MongoClient.connect(uri,{ useUnifiedTopology: true }),
            ...
            process = function(clientName,pathName){
                ...
                dispatchCollection.aggregate([
                    ...
                    { $unwind: "$dGeofence" }, // do not preserveNull. geofence is required
                    ...
                    if(docs.length > 0){
                        for(var i = 0; i < docs.length; i++){
                            ...
                            doc.dGeofence = doc.dGeofence || {}; // because of $unwind
                        ...
                        Object.keys(sites).forEach(key => {
                            sites[key].forEach(doc => {
                                ...
                                if(doc.status == "queueingAtOrigin"){
                                    // LONG QUEUEING
                                    if(doc.event_time && doc.event_time.queueingAtOrigin != null){
                                        var queueing_time = getTimestamp() - getTimestamp(doc.event_time.queueingAtOrigin),
                                ...
                                if(["queueingAtOrigin","processingAtOrigin"].includes(doc.status) && !delayIdentified){
                                    // OVER CICO
                                    if(doc.event_time && (doc.event_time.processingAtOrigin != null || doc.event_time.queueingAtOrigin != null || doc.event_time.entered_origin != null)){
                                        var cico_time = getTimestamp() - getTimestamp(doc.event_time.entered_origin || doc.event_time.queueingAtOrigin || doc.event_time.processingAtOrigin),
                                ...
                                if(_docs.length > 0){
                                    if(hasDelay.escalation01 === true){
                                        Object.keys(siteTable[1]).forEach(sKey => {
                                            ...
                                            if(assigned.geofence[sKey].length > 0){
                                                assigned.geofence[sKey].forEach(username => {
                                                    ...
                                                    if(user && user.email){
                                                        ...
                                                        emailDetails.push({
                                                            ...
                                                            subject: `Escalation 01 at ${sKey}`,
                                                            content: escalation01(defaultUser,siteTable[1][sKey])
                                                        });
                                                    }
                                                });
                                            } else {
                                    ...
                                    if(hasDelay.escalation02 === true){
                                        Object.keys(siteTable[2]).forEach(sKey => {
                                            ...
                                            if(assigned.cluster[sKey].length > 0){
                                                assigned.cluster[sKey].forEach(username => {
                                                    ...
                                                    if(user && user.email){
                                                        ...
                                                        emailDetails.push({
                                                            ...
                                                            to: defaultUser.email,
                                                            subject: `Escalation 02 at ${sKey}`,
                                                            content: escalation02_03(defaultUser,siteTable[2][sKey],2)
                                                        });
                                                    }
                                                });
                                            } else {
                                    ...
                                    if(hasDelay.escalation03 === true){
                                        Object.keys(siteTable[3]).forEach(sKey => {
                                            ...
                                            if(assigned.region[sKey].length > 0){
                                                assigned.region[sKey].forEach(username => {
                                                    ...
                                                    if(user && user.email){
                                                        ...
                                                        emailDetails.push({
                                                            ...
                                                            to: defaultUser.email,
                                                            subject: `Escalation 03 at ${sKey}`,
                                                            content: escalation02_03(defaultUser,siteTable[3][sKey],3)
                                                        });
                                                    }
                                                });
                                            } else {
                    ...
                    function loopThroughMailDetails(){
                        ...
                        if(emailDetails.length > 0){
                            console.log("Temporary disabled email.");
                            proceedToUpdate();
