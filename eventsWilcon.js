...
exports.eventsWilcon = (req, res) => {
    ...
    try {
        ...
        request({
            method: 'POST',
            url: `https://asia-east2-secure-unison-275408.cloudfunctions.net/eventsWilconDev?${queryString}`,
            headers: {
                                ...
                                for(var i = 0; i < docs.length; i++){
                                    ...
                                    if(!doc.scheduled_date || (moment().valueOf() >= moment(doc.scheduled_date).valueOf())){
                                        ...
                                        if(roundtrip == true){
                                            ...
                                            if(query.RULE_NAME == "Inside Geofence" && isOrigin === true && !timestampEnteredOrigin && doc.status == "in_transit"){
                                                var inTransitTimestamp = null;
                                                Object.keys(doc.events_captured||{}).forEach(key => {
                                                    if(doc.events_captured[key] == "in_transit") {
                                                        inTransitTimestamp = key;
                                                    }
                                                });
                                                if(inTransitTimestamp){
                                                    var start = moment(Number(inTransitTimestamp));
                                                    var end = moment();
                                                    var minuteDifference = end.diff(start, "minutes");
