...
exports = module.exports = functions.region('asia-east2').runWith({ timeoutSeconds: 60, memory: '128MB' }).https.onRequest((req, res) => {
    ...
    try { request({ method: 'GET', url: `https://asia-east2-secure-unison-275408.cloudfunctions.net/pmsAlertxDev` }); } 
    ...
    co(function*() {
        ...
        const CLIENT_OPTIONS = {
            "wm-wilcon": { otherDb: "wilcon", alertBasis: 'date-based' },
            "wm-odo": { otherDb: "odo", alertBasis: 'odo-based' }
        };
        ...
        function process(clientName){
            ...
            // Alert Basis of the client
            const alertBasis = CLIENT_OPTIONS[clientName].alertBasis;

            // alerting the users with this number of days before the next scheduled date
            const alertDaysBefore = 3;

            // max increment of odometer to send alert again
            const odoIncrementalAlert = 10000;
                ...
                pmsRequestsCollection.aggregate([
                    ...
                    if(docs.length > 0){
                        ...
                        docs.forEach(val => {
                                    ...
                                    if(diffDays == alertDaysBefore){
                                        ...
                                        (val.emails||[]).forEach(to => {
                                            ...
                                            childPromise.push(transporter.sendMail({
                                                ...
                                                text: emailTemplate[alertBasis](months,last_sc_date,next_sc_date),
                                                html: emailTemplate[alertBasis](months,last_sc_date,next_sc_date),
                                            }));
                                        });
                            ...
                            if(alertBasis == 'odo-based') {

                                // make sure that the vehicle exists in the database and there's a PMS linked to the request
                                if(vehicle && val.odometer_alert){
        
                                    // get last odometer value
                                    const last_odometer = val.last_odometer;
                                    // get current odometer value
                                    const current_odometer = vehicle.odometer || 0; //55000;
                                    // get odometer alert value
                                    const odometer_alert = val.odometer_alert;

                                    console.log("Odometers",last_odometer,current_odometer,odometer_alert,"ID:",val._id);
        
                                    // if its alertDaysBefore(3) days before the next scheduled date, alert users
                                    if((!last_odometer && current_odometer >= odometer_alert) || 
                                       (last_odometer && current_odometer >= (odometer_alert + odoIncrementalAlert))){
                                        ...
                                        (val.emails||[]).forEach(to => {
                                            childPromise.push(transporter.sendMail({
                                                from: '"WRU Maintenance" <noreply@wru.ph>', // sender address
                                                to: to || `wru.developer@gmail.com`, // list of receivers
                                                subject: `Odometer Alert - ${vehicle.name}`, // Subject line
                                                text: emailTemplate[alertBasis](vehicle.name,odometer_alert,current_odometer),
                                                html: emailTemplate[alertBasis](vehicle.name,odometer_alert,current_odometer),
                                            }));
                                        });
        
                                        // update this pms' next scheduled date and last scheduled date, and status
                                        childPromise.push(pmsRequestsCollection.updateOne({_id: val._id}, {   
                                            $set: { last_odometer: current_odometer }
                                        })); 
                                    } 
                                } 
                            }
        ...
        const emailTemplate = {
            'date-based': ( months, last_sc_date, next_sc_date ) => {
            ...
            'odo-based': ( vehicle, odometer_alert, current_odometer ) => {
                return `<html lang="en">
                            <head>
                                <style>
                                    body {
                            ...
                            <body>
                                <div>Good day,</div>
                                <br>
                                <div>This is a reminder of the Preventive Maintenance that the odometer of ${vehicle} has reached or exceeded <b>${Number(odometer_alert).toLocaleString('en')} kms</b>.</div>
                                <div>Odometer at current timestamp: <b>${Number(current_odometer).toLocaleString('en')} kms</b></div>
                                <br>
                                <br>
                                <div>Thank you!</div>
                                <div><hr style="border: 0;border-top: 1px solid #eee;margin: 20px 0px;"></div>
